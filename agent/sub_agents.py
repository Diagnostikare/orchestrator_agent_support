...
_TICKET_RULES = (
    ...
    "- `motivo` y `submotivo` los clasificas TU a partir de lo que conto el "
    "usuario, con los valores exactos que describe la tool. Son etiquetas para "
    "agrupar el reporte, no algo que se le pregunte: si de la conversacion no "
    "se desprende cual es, manda \"otro\" y el submotivo vacio. No detengas la "
    ...
    "- Al contarle un ticket, usa el `user_summary` que devuelve `ver_ticket`: "
    "esta escrito para el. El `enhanced_body` es la nota interna del equipo, "
    "no se la leas literal; si el ticket no trae `user_summary`, resumelo tu "
    "en lenguaje llano y sin jerga.\n"
